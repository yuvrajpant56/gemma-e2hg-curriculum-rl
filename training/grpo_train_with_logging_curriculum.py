...
try:
    ...
    CONFIG_LOG_COMPLETION_DETAILS = True
...
class PatchedGRPOTrainer(GRPOTrainer):
    def _get_train_sampler(self, train_dataset=None):
        """Use the curriculum-aware TaskSampler instead of TRL default sampler.

        GRPO max_steps counts optimizer updates, but the DataLoader samples one
        batch per microstep. With gradient accumulation, the curriculum should
        span max_steps * gradient_accumulation_steps microsteps.
        """
        ...
        return TaskSampler(
            ...
            scheduler_params=getattr(
                self.args,
                "scheduler_params",
                {"curriculum_beta": 0.5, "sigma": 0.5, "min_prob": 0.0},
            ),
            seed=self.args.seed,
...
def main() -> None:
    ...
    output_dir = os.environ.get("OUTPUT_DIR", "outputs/grpo_countdown_task_sampler")
    run_name = os.environ.get("RUN_NAME", "grpo_countdown_task_sampler")
    ...
    # Custom attributes consumed by PatchedGRPOTrainer._get_train_sampler().
    # These mirror grpo_train.py while keeping the debug file inside this run logs.
    training_args.data_schedule = os.environ.get("DATA_SCHEDULE", "gaussian")
    training_args.scheduler_params = {
        "curriculum_beta": float(os.environ.get("CURRICULUM_BETA", "0.5")),
        "sigma": float(os.environ.get("CURRICULUM_SIGMA", "0.5")),
        "min_prob": float(os.environ.get("CURRICULUM_MIN_PROB", "0.0")),
    }
    training_args.debug_sampler = bool_from_env("DEBUG_SAMPLER", True)
    training_args.debug_sampler_every = int(os.environ.get("DEBUG_SAMPLER_EVERY", "5"))
    training_args.debug_sampler_file = os.environ.get(
        "DEBUG_SAMPLER_FILE",
        str(Path(paths.logs_dir) / "sampler_task_debug.tsv"),
    )

    experiment_config.update({
        "data_schedule": training_args.data_schedule,
        "scheduler_params": training_args.scheduler_params,
        "debug_sampler": training_args.debug_sampler,
        "debug_sampler_every": training_args.debug_sampler_every,
        "debug_sampler_file": training_args.debug_sampler_file,
    })
    with open(paths.experiment_config_json, "w") as f:
        json.dump(experiment_config, f, indent=2)
